...
const germanUppercaseLetters = Object.keys(LETTER_SEGMENTS);

function createGermanLatinTemplates(caseStyle: "uppercase" | "lowercase"): WritingTemplate[] {
  return germanUppercaseLetters.map((letter) => {
    const lowercaseLetter = letter.toLowerCase();
    const segments = LETTER_SEGMENTS[letter];
    const idPrefix = caseStyle === "uppercase" ? "de" : "de-lower";
    ...
    return {
      ...
      cue,
      description: {
        ko: `독일어 알파벳 ${nativeLabel} 연습용 임시 템플릿입니다.`,
        en: `Practice template for the German letter ${nativeLabel}.`,
      },
      direction: "ltr",
...
export const deLatinAlphabetTemplates: WritingTemplate[] = [
  ...createGermanLatinTemplates("uppercase"),
  ...createGermanLatinTemplates("lowercase"),
];
