...
const frenchUppercaseLetters = Object.keys(LETTER_SEGMENTS);

function createFrenchLatinTemplates(caseStyle: "uppercase" | "lowercase"): WritingTemplate[] {
  return frenchUppercaseLetters.map((letter) => {
    const lowercaseLetter = letter.toLowerCase();
    const segments = LETTER_SEGMENTS[letter];
    const idPrefix = caseStyle === "uppercase" ? "fr" : "fr-lower";
    ...
    return {
      ...
      cue,
      description: {
        ko: `프랑스어 알파벳 ${nativeLabel} 연습용 임시 템플릿입니다.`,
        en: `Practice template for the French letter ${nativeLabel}.`,
      },
      direction: "ltr",
...
export const frLatinAlphabetTemplates: WritingTemplate[] = [
  ...createFrenchLatinTemplates("uppercase"),
  ...createFrenchLatinTemplates("lowercase"),
];
