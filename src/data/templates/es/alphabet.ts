...
const spanishUppercaseLetters = Object.keys(LETTER_SEGMENTS);

function createSpanishLatinTemplates(caseStyle: "uppercase" | "lowercase"): WritingTemplate[] {
  return spanishUppercaseLetters.map((letter) => {
    const lowercaseLetter = letter.toLowerCase();
    const segments = LETTER_SEGMENTS[letter];
    const idPrefix = caseStyle === "uppercase" ? "es" : "es-lower";
    ...
    return {
      ...
      cue,
      description: {
        ko: `스페인어 알파벳 ${nativeLabel} 연습용 임시 템플릿입니다.`,
        en: `Practice template for the Spanish letter ${nativeLabel}.`,
      },
      direction: "ltr",
...
export const esLatinAlphabetTemplates: WritingTemplate[] = [
  ...createSpanishLatinTemplates("uppercase"),
  ...createSpanishLatinTemplates("lowercase"),
];
